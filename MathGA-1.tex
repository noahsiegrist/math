...
Diese könne natürlich auch verneint werden:
\begin{itemize}
  \item $\neg \forall x : P(x))$ Für \textbf{alle} x ist $P(x)$ wahr.
  \item $\exists x : P(x)$ Für \textbf{mindestens} ein x ist $P(x)$ wahr.
  \item $\exists! x : P(x)$ Für \textbf{genau} ein x ist $P(x)$ wahr.
\end{itemize}
...
Objekte / Gegenstände / Ideen, worüber wir sprechen wollen (z.B. mit Logik) -> dies bietet die Mengenlehre
