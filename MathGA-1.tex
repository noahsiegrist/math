...
\fancyhead[RO]{\nouppercase{\leftmark}}
\lhead{Mariel Alaba, Jan Meier, Noah Siegrist}
\fancyfoot[LE,RO]{Seite \centering{\thepage} | Mathe GA-1}
...
Eine (logische) Aussage kann immer als \textbf{wahr (w)} order \textbf{falsch (f)} identifiziert werden. Wenn die Aussage nicht \textbf{eindeutig} mit w oder f beantwortet werden kann ist diese Aussage keine logische Aussage.
...
\begin{itemize}
  ...
  \item Feuer kühlt. (f)
  \item Feuer ist schön. (subjektiv aber bestimmbar)
...
Logische Aussagen können verknüpft werden mittels aussagenlogischer Verknüpfungen oder auch “Konnektoren”, “Junktoren” oder “Operatoren” gennant. Diese Verknüpfungen können jeweils mit \textbf{Wahrheitstabellen} definiert werden.
...
\subsubsection{Konjunktion (und) $\land$}
...
\subsubsection{Disjunktion (oder) $\lor$}
...
Beim logische oder $(\lor)$ ist anzumerken, dass es sich vom “entweder oder” unterschiedet, sofern beide Seiten korrekt sind wird auch das Ergebnis korrekt sein (wahr).
...
Sie bindet am dritt stärksten nach der Konjunktion und der Disjunktion. Eine Gesamtübersicht zu Bindungsstärken findet sich in der Mengenlehre zum Schluss nochmals.
\subsubsection*{Wahrheitstabelle}
\begin{tabular}{c|c || c}
  A & B & \( A \implies B \) \\
  \hline
...
Eine atomare Aussage ist eine Aussage ohne jegliche Verknüpfungen oder Negationen. Bei der aussagelogischen Formel ( \(f = A \land \neg B \land B\) ) sind die atomaren Aussagen ( \(A\) und \(B\) ).

\subsection{Literal}
Ein Literal ist eine Aussage ohne jegliche Verknüpfungen. Im Gegensatz zur atomaren Aussage sind hier Negationen nicht ausgeschlossen. Also sind bei der aussagelogischen Formel ( \(f = A \land \neg B \land B\) ) die Literale ( \(A\), \(B\) und \(\neg B\) ).
...
Regeln wie elementare Zeichen (Symbole) – oder auch Symbolgruppen – korrekt zusammengesetzt werden dürfen. 
...
In der KNF ist es das Ziel, dass im obersten Level der aussagelogischen Formel nur Konjunktionen vorhanden sind. Diese halten so genannte “Klauseln der konjunktiven Normalform” zusammen.
\begin{displaymath}
...
In der DNF ist es das Ziel, dass im obersten level der Aussagelogischen Formel nur Disjunktionen vorhanden sind. Diese halten so genannte “Klauseln der disjunktiven Normalform” zusammen.
\begin{displaymath}
...
Es wird eine aussagenlogische Formel \textbf{erfüllbar} genannt, wenn mindestens ein Modell in der Wahrheitstabelle gefunden werden kann. Falls kein Modell gefunden werden kann handelt es sich um eine \textbf{Kontradiktion / Widerspruch}. \textit{Das warme Feuer ist kalt.}

Falls in der Wahrheitstabelle nur Modelle gefunden werden können nennen wir die aussagenlogische Formel eine \textbf{Tautologie}. \textit{Das warme oder kalter Feuer.} 
...
Unser Prädikat \( P(x) \) wird erst zur Aussage, wenn für x ein Objekt eingesetzt wird. Solange kein Objekt eingesetzt ist, kann das Prädikat auch nicht auf wahr oder falsch definiert werden und ist deshalb auch noch keine Aussage. Wenn nun ein Rechteck für x eingesetzt wird, können wir feststellen, dass die Aussage \textit{"Rechteck ist rund"} falsch ist.
...
\begin{itemize}
  ...
  \item \(Z() = \) Ein Prädikat ohne Variablen ist eine Aussage.
...
$A$ := \{2, 4\} \\
$B$ := \{8, 16\} \\
$A \times B$ := \{$(2,8), (2,16), (4,8), (4,16)$\}

$A \times B$ := \{$(x,y) | x \in A \land y \in B$\} \\
$A$ (:= Bewohner) := \{Bewohner1, Bewohner2\} \\
$B$ (:= Waschtage) := \{Dienstag, Donnerstag\} \\
$A \times B$ := \{(Bewohner1, Dienstag), (Bewohner1, Donnerstag), (Bewohner2, Dienstag), (Bewohner2, Donnerstag)\}
